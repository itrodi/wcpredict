...
| `/strategies` | dynamic (searchParams) | per-matchday **Safe / Medium / Risky** board (`lib/strategies.ts`, `buildRiskTiers`) shown side-by-side: each tier draws legs from a model-probability band (safe ≥ 60%, medium 42–60%, risky 25–42%) and surfaces singles, cross-match accumulators (double/treble/4-fold) and **correlation-aware same-game combos** across every market (result, goals overs, BTTS, corners, halves). Inside a tier the picks are value-ranked (probability × odds) so positive-edge book prices rise to the top; riskier tiers use a looser same-game joint floor. Same-game pairs price their dependence with a Gaussian copula (`lib/stats.ts` + `lib/correlation.ts`) so the joint beats the naive product; each bet carries edge/EV, a flat-1u baseline and a fractional-Kelly stake. **Responsible-gambling warning lives on this page** |
